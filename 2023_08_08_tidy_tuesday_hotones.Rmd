...
super_hot <- median(tt$sauces$scoville)
...
sauce_rec <- recipe(super_hot ~ sauce_name, data = sauce_train) %>%
  ...
  step_tokenfilter(sauce_name, max_tokens = 200) %>%
...
final_lasso %>%
  ...
  tt_theme() +
  labs(
    x = "Importance of word (based on a Lasso Regression model with a ROC AUC of 0.713)",
    y = NULL,
