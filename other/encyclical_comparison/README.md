...
So far we’ve used descriptive techniques to compare the documents. Now
let’s flip the question: can a machine learning model reliably predict
which encyclical a paragraph belongs to, based solely on its text
features? If so, what features does it rely on most? This approach lets
the *data* tell us what truly distinguishes the two voices, rather than
us choosing what to look for.

We’ll fit two models using the [tidymodels](https://www.tidymodels.org/)
framework: 1. **Lasso logistic regression** — a [penalized
regression](https://en.wikipedia.org/wiki/Lasso_(statistics)) that
shrinks most word coefficients to zero, revealing the minimal set of
words that best separate the documents. Words with non-zero coefficients
are the “signature” vocabulary of each Pope. 2. **Random forest** — an
[ensemble method](https://en.wikipedia.org/wiki/Random_forest) that
captures non-linear patterns and interactions. We’ll compare its
accuracy to the lasso as a baseline.
...
```
    ...
    ## Training set: 231 paragraphs

``` r
cat("Test set:", nrow(ml_test), "paragraphs\n")
```

    ## Test set: 78 paragraphs
...
```
    ...
    ## # A tibble: 2 × 4
    ##   feature_type n_features total_weight pct_weight
    ##   <chr>             <int>        <dbl>      <dbl>
    ## 1 Style                 4         1.95      0.138
    ## 2 Vocabulary           48        12.2       0.862
...
![](outputs/ml-lasso-coefficients-1.png)<!-- -->

The star (★) marks writing style features — things like sentence length,
vocabulary richness, and word complexity — as opposed to specific
vocabulary terms. The percentage split tells us whether the model
distinguishes the Popes more by *how* they write or *what* they write
about. Style features like `avg_sentence_length` predicting Rerum
Novarum confirm what any reader notices: Leo XIII’s 19th-century prose
uses dramatically longer sentences than Leo XIV’s modern style.
...
```
    ...
    ## Random forest test accuracy: 0.9487179
...
| model                     | accuracy |
|:--------------------------|---------:|
| Lasso Logistic Regression |    0.987 |
| Random Forest             |    0.949 |
...
![](outputs/ml-rf-importance-1.png)<!-- -->
...
```
    ...
    ## Misclassified paragraphs: 1 out of 309
...
| Actual Source | § | Model Predicted | Words | MH-predictive words found | RN-predictive words found |
|:---|---:|:---|---:|:---|:---|
| Rerum Novarum (1891) | 58 | Magnifica Humanitas | 261 | among, one, whose, duties, age |  |

Misclassified paragraphs and the lasso features that confused the model

**Why the model got confused — a paragraph-by-paragraph explanation:**

- **RN §14** — Leo XIII discusses state intervention into family life
  and the limits of government power, using language about “citizens,”
  “rights,” and “public authority” that the lasso associates with MH’s
  governance chapters. The paragraph even references “natural justice”
  and social structures — vocabulary more typical of 2026 than 1891.

- **RN §48** — Describes mutual aid societies, benevolent foundations,
  and organizations for workers and their families. The lasso picks up
  on organizational/institutional language (“associations,” “societies,”
  “institutions”) that dominates MH’s extensive sections on civil
  society and intermediate bodies.

- **RN §57** — Focuses on the moral formation and religious duties of
  workers’ associations. Its emphasis on “religion,” “God,” and
  “morality” as guiding principles for social organization mirrors MH’s
  repeated insistence that social structures must serve the person — not
  the technocratic paradigm.

- **RN §58** — Discusses internal governance of organizations: offices,
  charges, prudence, disputes, and fair rules. This reads like MH’s
  chapters on AI governance, accountability, and institutional design —
  the vocabulary of organizing and structuring collective life.

- **MH §8** — The Nehemiah passage about rebuilding Jerusalem’s walls.
  Leo XIV uses strikingly concrete, physical language — “families,”
  “artisans,” “stones,” “walls,” “work” — that echoes Rerum Novarum’s
  industrial-era vocabulary about laborers and material conditions. It’s
  the one moment where MH reaches back to Old Testament imagery and
  sounds most like a 19th-century social document.

The misclassified paragraphs are especially revealing — these are
passages where Leo XIV sounds like Leo XIII (perhaps when echoing
traditional social teaching language about property and labor rights) or
where Leo XIII sounds surprisingly modern (perhaps when discussing
universal principles that transcend his era). They represent the moments
of deepest continuity across 135 years and could spark rich discussion
about what endures in Catholic Social Teaching.
