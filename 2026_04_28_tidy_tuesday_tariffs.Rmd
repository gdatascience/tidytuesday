...
The R version works, but for social media sharing I wanted something more polished — dark background, product photography instead of emoji, and an editorial magazine aesthetic. I used **Google Nano Banana Pro** to transform the R-generated chart into the final version.
...
3. **Wrote a detailed design prompt** for Google Nano Banana Pro specifying: dark background, product photography in circular cutouts replacing emoji, glowing Liberation Day bars, editorial typography, and Instagram-ready 1080×1350 format
...
The final version replaces emoji with real product photos (tuna cans, cheese wedges, strawberries, carrots), uses a dark navy background for contrast, and adds a subtle glow effect to the Liberation Day bars to create visual urgency. The data is identical — only the presentation changed.

```{r final-image, eval=FALSE}
