...
```{r save-image}
# Final shareable image: bar chart with Liberation Day, emojis, and USITC seal
# Built in two stages:
# 1. Render the base plot with showtext (Google Fonts + Font Awesome caption)
# 2. Composite emoji PNGs + seal with magick
...
for (nm in names(emoji_list)) {
  ...
  # Trim and add padding to prevent visual cropping
...
# Stage 1: Build the base bar chart
final_plot <- stacked %>%
...
ggsave(
  ...
  final_plot, device = "png", width = 8, height = 10, dpi = 300, bg = "white"
)

# Stage 2: Composite emojis + seal with magick
base_img <- image_read(file.path(spec_dir, "_base_v3.png"))
r <- as.raster(base_img)
n_rows <- nrow(r)

# Find caption (scan from bottom up)
caption_start <- n_rows
for (row in seq(n_rows, n_rows - 200, by = -1)) {
  nw <- which(r[row, ] != "#ffffffff")
  if (length(nw) > 10) caption_start <- row
}

# Find strip text end (scan upward from caption through the gap)
strip_end <- 0
in_gap <- FALSE
...
# Place emojis centered in the gap between strip text and caption
...
image_write(result, "2026_04_28_tidy_tuesday_tariffs.png")
```
